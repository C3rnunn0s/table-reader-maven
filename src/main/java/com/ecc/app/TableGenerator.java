...
class TableGenerator{
    ...
    public void saveTable() throws IOException{        
        ...
        for(Map.Entry<Integer,HashMap<Integer, String>> row : table.entrySet()){
            for(Map.Entry<Integer, String> column : row.getValue().entrySet()){
                String input = String.join("", String.join("",row.getKey().toString(),",",column.getKey().toString()), ":", column.getValue(), "\n");
                try{ 
                    Files.write(this.path, input.getBytes(), StandardOpenOption.APPEND);
                }catch(IOException e){
                    System.out.println("File not found");
    ...
    public void printTable() throws IOException{
        ...
        for(Map.Entry<Integer,HashMap<Integer, String>> row : table.entrySet()){
            System.out.print("|  ");
            for(Map.Entry<Integer, String> column : row.getValue().entrySet()){
                System.out.print(column.getValue().toString().substring(0, stringLen) + " : " + column.getValue().toString().substring(stringLen) + "  |  ");
            }
            System.out.println();
        }
    ...
    public void searchTable() throws IOException{
        ...
        for(Map.Entry<Integer,HashMap<Integer, String>> row : table.entrySet()){
            for(Map.Entry<Integer, String> column : row.getValue().entrySet()){
                ...
                /*for(int a = 0; a <= table.getStringLen()*2 - searchLen; a++){
                    int b = a + searchLen;
                    if(search.compareToIgnoreCase(column.getValue().substring(a, b)) == 0){
                        hasMatch = true;                    
                        counter += 1;                    
                    }
                }*/
                counter = StringUtils.countMatches(column.getValue(), search);
                //if(hasMatch) System.out.println("(" + row.getKey() + "," + column.getKey() + ") - " + counter);
                if(counter > 0) System.out.println("(" + row.getKey() + "," + column.getKey() + ") - " + counter);
                hasMatch = false;
                counter = 0;         
            }
        }
